...
from lib.agent import DeepQAgent, RecurrentDeepQAgent
import numpy as np
import os
import argparse
from nn import RNN
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--save', help='The path of the directory where to save results.',
                        type=str, default='results/multiagent_mdp_recurrent')
    ...
    agent1 = RecurrentDeepQAgent(RNN(), state_space=state_space, update_frequency=update_frequency,
                                norm=norm, memory_size=memory_size, seq_length=seq_length)
    agent2 = RecurrentDeepQAgent(RNN(), state_space=state_space, update_frequency=update_frequency,
                                norm=norm, memory_size=memory_size, seq_length=seq_length)
